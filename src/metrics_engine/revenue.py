...
def calculate(df, metrics):
    ...
    aov = metrics["Total Revenue"] / metrics["Total Conversions"] if metrics["Total Conversions"] else 0
    ...
    metrics["Marketing ROI"] = (
        round(((metrics["Total Revenue"] - metrics["Total Spend"]) / metrics["Total Spend"]) * 100, 2)
        if metrics["Total Spend"]
        else 0
    )
    metrics["Revenue Per Click"] = (
        round(metrics["Total Revenue"] / metrics["Total Clicks"], 2) if metrics["Total Clicks"] else 0
    )
    if metrics["CPA"]:
