...
def calculate(df, metrics):
    ...
    total_engagements = df["likes"].sum() + df["comments"].sum() + df["shares"].sum()
    ...
    metrics["Average Bounce Rate"] = (
        round(float(df["bounce_rate"].mean()), 2) if "bounce_rate" in df else None
    )

    metrics["Average Frequency"] = (
        round(float(df["frequency"].mean()), 2) if "frequency" in df else None
    )
