...
class InsightQualityChecker:
    ...
    def __init__(self, analysis: dict, campaign_row: dict):
        ...
        self._confidence = self._coerce_float(
            self.analysis.get("confidence_score", 0), 0.0
        )
    ...
    def check_factual_grounding(self) -> CriterionResult:
        ...
        partial_ok = (
            has_numbers and len(grounded) < len(key_signals) and len(grounded) > 0
        )
        if all_ok:
            ...
            detail = (
                ...
                f"Missing data in: {ungrounded[0][:50]}..."
                if ungrounded
                else ""
            )
        else:
    ...
    def check_benchmark_specificity(self) -> CriterionResult:
        ...
        text = text.lower()
        has_benchmark = bool(
            re.search(r"(benchmark|threshold|above|exceeds)\D{0,20}\d", text)
        )
    ...
    def check_issue_detection(self) -> CriterionResult:
        ...
        if flag_freq:
            triggers.append(
                f"frequency={self._frequency} > 2.5 " f"(Instagram fatigue threshold)"
            )
        if flag_bounce:
            triggers.append(f"bounce_rate={self._bounce_rate} > 0.35")
    ...
    def check_confidence_calibration(self) -> CriterionResult:
        ...
        if (
            self.analysis.get("detected_issues") == []
            and self.analysis.get("confidence_score") > 80
        ):
            return CriterionResult(
