...
class DecisionInterpreter:
    ...
    def _compute_confidence(self, final_score, business, compliance, gt):
        ...
        if recs:
            rec_avgs = [sum(r["scores"].values()) / len(r["scores"]) for r in recs]
    ...
    def _build_decision(
        self, final_score: float, gt: Dict, business, compliance
    ) -> Dict:

        gt_score = gt.get("score", 0) if gt else 0

        if (
            final_score >= self.accept_threshold
            and gt_score >= self.gt_strong_threshold
        ):
            status = "ACCEPT"
        ...
        confidence = self._compute_confidence(final_score, business, compliance, gt)
    ...
    def _decision_reason(self, final_score, gt_score):
        ...
        if final_score > 0.8 and gt_score < 0.7:
            return (
                "High quality recommendations but weak alignment with expert strategy"
            )
    ...
    def _classify_recommendations(self, business: Dict) -> Dict:
        ...
        return {"keep": keep, "review": review, "revise": revise}
