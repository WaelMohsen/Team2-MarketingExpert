...
class MetricsCalculator:
    ...
    def run(self, campaigns, target: str) -> tuple:
        try:
            ...
            builders = {
                "acquisition": lambda: Acquisition_Metrics(*self.calculate_acquisition_metrics(campaigns)),
                "revenue":     lambda: Revenue_Metrics(*self.calculate_revenue_metrics(campaigns)),
                "retention":   lambda: Retention_Metrics(*self.calculate_retention_metrics(campaigns)),
                "satisfaction":lambda: Satisfaction_Metrics(*self.calculate_satisfaction_metrics(campaigns)),
            }

            if target not in builders:
                raise ValueError(f"Unknown target '{target}'. Choose from: {list(builders)}")

            return base, builders[target]()
        except ValueError as e:
            raise
        except Exception as e:
            raise RuntimeError(f"Failed to run metrics for target '{target}': {e}") from e

    def calculate_base_metrics(self, campaigns) -> tuple[float, float, int, int, int, int]:
        try:
            self.validate_campaigns(campaigns)
            ...
            return total_spend, total_revenue, total_impressions, total_clicks, total_conversions, total_new_customers
        except ValueError:
            raise
        except Exception as e:
    ...
    def calculate_revenue_metrics(self, campaigns) -> tuple[float, float, float, float, float, float]:
        try:
            self.validate_campaigns(campaigns)
            ...
            marketing_roi = round(((total_revenue - total_spend) / total_spend) * 100, 2) if total_spend else 0
            revenue_per_click = round(total_revenue / total_clicks, 2) if total_clicks else 0
            ltv_cac_ratio = round(annual_customer_value / cpa, 2) if cpa else 0
            return roas, aov, annual_customer_value, marketing_roi, revenue_per_click, ltv_cac_ratio
        except ValueError:
            raise
        except Exception as e:
    ...
    def calculate_satisfaction_metrics(self, campaigns) -> tuple[float, float, float, float]:
        try:
            self.validate_campaigns(campaigns)
            total_reach = sum(c.reach for c in campaigns)
            total_engagements = sum(c.likes + c.comments + c.shares for c in campaigns)
            engagement_rate = round((total_engagements / total_reach) * 100, 2) if total_reach else 0
            avg_bounce_rate = round(sum(c.bounce_rate for c in campaigns) / len(campaigns), 2) if campaigns else 0
            avg_frequency = round(sum(c.frequency for c in campaigns) / len(campaigns), 2) if campaigns else 0
