...
class MetricsCalculator:
    ...
    def calculate_acquisition_metrics(self, campaigns) -> tuple[float, float, float]:
        try:
            ...
            (
                ...
                total_conversions,
                total_new_customers,
            ) = self.calculate_base_metrics(campaigns)
            ctr = (
            ...
            cpa = (
                round(total_spend / total_new_customers, 2)
                if total_new_customers
                else 0
            )
    ...
    def calculate_revenue_metrics(
        ...
        try:
            self.validate_campaigns(campaigns)
            total_spend, total_revenue, _, total_clicks, total_conversions, _ = (
                self.calculate_base_metrics(campaigns)
            )
